function validateForm(email, pw, pw1, varicode) {
    
    if (email == "" || pw == "" || pw1 == "" || varicode == "") {
        window.alert("输入不可留空！")
        return false
    } else if (pw1 != pw) {
...
function signUp(form) {
    ...
    let varicode = formData.get("varicode")
    let xsrf = formData.get("_xsrf")

    if (validateForm(email, pw, pw1, varicode) == true) {
        var xmlhttp;
        ...
        data = "_xsrf="+xsrf+"&email="+email+"&pw="+pw+"&varicode="+varicode
